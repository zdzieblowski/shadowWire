...
server.post("/write", (req, rep) => {
  ...
  if (fs.existsSync(messages_path)) {
    let data = JSON.parse(fs.readFileSync(messages_path));
    //data.messages.push({"height":data.messages.length, "date": req.body.date, "data": req.body.message, "sender": req.body.sender});
    ...
    //let modified_data = JSON.stringify({"messages": [{"height": 0, "date": req.body.date, "data":req.body.message, "sender": req.body.sender}]});
